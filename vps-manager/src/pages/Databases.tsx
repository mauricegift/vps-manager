      ...
      <table className="vps-table text-[11px] min-w-max">
        <thead className="sticky top-0 bg-[var(--secondary)] z-10">
          <tr>
            ...
            {canEdit && <th className="px-3 py-2 border-b border-[var(--line)] text-right w-16">Actions</th>}
          </tr>
        </thead>
        <tbody>
          {data.rows.length === 0 ? (
            <tr><td colSpan={data.columns.length + (canEdit ? 1 : 0)} className="text-center py-6 text-[var(--muted)] italic">No rows found</td></tr>
          ) : (
            data.rows.map((row, i) => (
