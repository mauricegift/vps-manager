...
- Browse databases, tables, and row data
- Run custom SQL queries with output preview
- Editable row table — edit/delete row action buttons always visible (no hover required)
- Wider output modals for comfortable reading
...
- Remote SSH terminal receives the same color environment automatically on connect
...
### Nginx + SSL
- Real-time nginx status display (running/stopped/version)
- List all config files — enable, disable, delete, view, or edit
- Create new configs from templates:
  - Static site, Reverse proxy, PHP-FPM, SSL redirect
...
- Full remote server support via SSH
...
6. Start the app via PM2
7. Optionally set up an SSL certificate via Certbot
...
| `DATABASE_URL` | PostgreSQL connection URL | auto (SQLite-like local DB) |
...
│   └── routes/             # pm2, docker, databases, files, nginx, extras, remote, vps-connections
