...
if [[ -z "${DOMAIN:-}" ]]; then
  ...
  # Set HTTP origin for this domain right away (may be upgraded to HTTPS below)
  ...
  if [[ "$DNS_OK" == false ]]; then
    ...
    if $SUDO certbot certonly --webroot -w /var/www/html \
      ...
      # ── Write HTTPS origin to .env so Express CORS allows the domain ─────────
      # Vite builds use <script type="module"> which browsers load with an
      # Origin header — without ALLOWED_ORIGIN the CORS middleware rejects it
      # and Express returns 500 on the JS bundle (blank page).
      if grep -q '^ALLOWED_ORIGIN=' "$ENV_FILE" 2>/dev/null; then
        sed -i "s|^ALLOWED_ORIGIN=.*|ALLOWED_ORIGIN=https://${DOMAIN}|" "$ENV_FILE"
      else
        echo "ALLOWED_ORIGIN=https://${DOMAIN}" >> "$ENV_FILE"
      fi
      log "ALLOWED_ORIGIN set to https://${DOMAIN} in .env"

      # ── Rewrite port-80 block: strip proxy, add redirect only ────────────────
      $SUDO tee "$NGINX_CONF" > /dev/null <<HTTP301EOF
# VPS Manager — generated by install.sh
map \$http_upgrade \$connection_upgrade {
  default upgrade;
  ''      close;
...
SSLEOF
      ...
      if $SUDO nginx -t; then
        ...
        warn "Nginx config test failed — HTTPS may not be active. Check: sudo nginx -t"
      fi

      # ── Block direct app port so all traffic must go via nginx ────────────────
      if command -v ufw &>/dev/null; then
        $SUDO ufw delete allow "$APP_PORT"/tcp >/dev/null 2>&1 || true
        log "Closed port $APP_PORT from firewall — all traffic now routes via nginx"
      fi

      # ── Restart PM2 app to pick up new ALLOWED_ORIGIN env var ────────────────
      pm2 restart "$APP_NAME" --update-env 2>/dev/null || true
      log "App restarted — HTTPS domain allowed in CORS"
      ...
      # SSL failed but domain IS known — allow HTTP domain origin so app works on port 80
      if grep -q '^ALLOWED_ORIGIN=' "$ENV_FILE" 2>/dev/null; then
        sed -i "s|^ALLOWED_ORIGIN=.*|ALLOWED_ORIGIN=http://${DOMAIN}|" "$ENV_FILE"
      else
        echo "ALLOWED_ORIGIN=http://${DOMAIN}" >> "$ENV_FILE"
      fi
      pm2 restart "$APP_NAME" --update-env 2>/dev/null || true
    fi
  fi
...
# ── If domain was entered but SSL was NOT issued, still update ALLOWED_ORIGIN ─
# Covers: DNS timeout, certbot failure (handled inline too — idempotent)
if [[ -n "${DOMAIN:-}" && "$SSL_ISSUED" == false ]]; then
  if ! grep -q '^ALLOWED_ORIGIN=' "$ENV_FILE" 2>/dev/null; then
    echo "ALLOWED_ORIGIN=http://${DOMAIN}" >> "$ENV_FILE"
    pm2 restart "$APP_NAME" --update-env 2>/dev/null || true
    log "ALLOWED_ORIGIN set to http://${DOMAIN} in .env"
  fi
fi
...
if [[ "$SSL_ISSUED" == true ]]; then
  echo -e "  ${BOLD}${GREEN}Access URL:${RESET}      ${BOLD}${ACCESS_URL}${RESET}  ${GREEN}← HTTPS secured${RESET}"
  echo -e "  ${BOLD}HTTP redirect:${RESET}   http://${DOMAIN}  →  ${ACCESS_URL}"
else
  echo -e "  ${BOLD}Access URL:${RESET}      ${ACCESS_URL}  (via Nginx on port 80)"
  if [[ -n "${DOMAIN:-}" ]]; then
    echo -e "  ${YELLOW}Note:${RESET} SSL was not issued — app accessible on HTTP only"
  fi
fi
echo -e "  ${BOLD}Direct port:${RESET}     http://${SERVER_IP}:${APP_PORT}$(  [[ "$SSL_ISSUED" == true ]] && echo "  ${YELLOW}(firewalled — use $ACCESS_URL)${RESET}" || echo '' )"
